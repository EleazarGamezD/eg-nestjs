...
In a new module, apply EG folders: `src/modules/monitoring/controller/`, `services/`, `module/`, and `middlewares/` for the custom middleware. Update imports when moving templates. Health indicators are infrastructure adapters. If the project already has this module, integrate without duplicating providers or routes. The reusable Bull Board helper can live in `src/core/config/bull-board/`; do not place it under WhatsApp in a project that does not have that domain.
...
- `/status` page, title, light background, responsive cards, Spanish login overlay from the exact EG HTML, and six Chart.js charts: CPU, RSS memory, mean response, event loop, traffic, and errors.
